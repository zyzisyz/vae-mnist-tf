...
>>>>>>> 768ecc505795127a1b33ec2879cc6fbdf4de17ff
    ...
    def decoder(self, z, is_training=True, reuse=False):
        ...
        with tf.variable_scope("decoder", reuse=reuse):
            net = tf.nn.relu(bn(linear(z, 1024, scope='de_fc1'),
                                is_training=is_training, scope='de_bn1'))
            ...
            out = tf.nn.sigmoid(
                deconv2d(net, [self.batch_size, 28, 28, 1], 4, 4, 2, 2, name='de_dc4'))
            return out
    ...
    def build_model(self):
        ...
        KL_divergence = 0.5 * tf.reduce_sum(tf.square(self.mu) + tf.square(
            sigma) - tf.log(1e-8 + tf.square(sigma)) - 1, [1])
    ...
    def visualize_results(self, epoch):
        ...
        if self.z_dim == 2:
            ...
            for idx in range(0, 100):
                ...
                batch_images = self.data_X[id *
                                           self.batch_size:(id + 1) * self.batch_size]
                batch_labels = self.data_y[id *
                                           self.batch_size:(id + 1) * self.batch_size]

                z = self.sess.run(self.mu, feed_dict={
